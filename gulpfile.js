...
(function(scope){
  ...
  //   var jsBundles = [distDir + '/scripts/buggyui.min.js'];
  ...
  var scriptsBrowserify = function(env){
    var jsBundles = ['app/scripts/buggyui.js'];
...
(function(scope){
  ...
  gulp.task('vulcanize:inline:now',function(){
    return gulp.src(distDir + '/buggyui.html', {base: distDir + '/'})
      .pipe($.vulcanize({
  ...
  var vulcanizeWithExternalScriptsAndStyles = function(vulcanizeOptions){
    return gulp.src(tmpDir + '/buggyui.html')
